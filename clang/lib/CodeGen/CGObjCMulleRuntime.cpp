                                          ...
                                          StringRef className)
{
   ...
   classID = _HashConstantForString( className);
...
{
   return( _HashConstantForString( PD->getIdentifier()->getName()));
}
                                                       ...
                                                       llvm::Constant *Prop[ 9])
{
   ...
   setterSel = (! setter.isNull() && ! PD->isReadOnly())  ? _HashConstantForString( setter.getAsString())
                                                          : zeroSel;
   adderSel  = (! adder.isNull() && ! PD->isReadOnly() && (PD->getPropertyAttributes() & ObjCPropertyDecl::OBJC_PR_relationship))  ? _HashConstantForString( adder.getAsString())
                                                         : zeroSel;
   removerSel= (! remover.isNull() && ! PD->isReadOnly() && (PD->getPropertyAttributes() & ObjCPropertyDecl::OBJC_PR_relationship))  ? _HashConstantForString( remover.getAsString())
                                                           : zeroSel;
...
//   unsigned Size = CGM.getDataLayout().getTypeAllocSize(ObjCTypes.CategoryTy);
   ...
   Values[ 2] = _HashConstantForString( Interface->getName());
   ...
   if (Category)
   {
      Values[ 0] = llvm::ConstantExpr::getBitCast( _HashConstantForString( Category->getName()), ObjCTypes.ClassIDTy);
   ...
   {
      Values[ 0] = llvm::ConstantExpr::getBitCast( _HashConstantForString( OCD->getName()), ObjCTypes.ClassIDTy);
...
//   };
   ...
   llvm::ConstantInt *ClassID = _HashConstantForString( ID->getObjCRuntimeNameAsString());
   ...
   if( Super)
   {
      SuperClassID = _HashConstantForString( Super->getObjCRuntimeNameAsString());
...
{
   return( _HashConstantForString( protocol->getName()));
}
